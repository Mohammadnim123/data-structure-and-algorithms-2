...
const templateWithJQuery = () => {
  ...
  starWarsPeople.forEach(element => {
   let secClone = $('#template').clone();
    secClone.find('h2').text(element.name);
    secClone.find('h3').text(element.height);
    secClone.find('p').text(element.eye_color);
    secClone.removeAttr('id');
...
for(var i=0 ; i<=str.length ; i++){
  var newstr = str.slice(i, str.length);
  result.push(newstr);
