...
function mapCurrentEvents (){
  let summaries = [];
 return get(currentEvents)
  .then(myData => {
  summaries = myData.map(val => {
...
const countNumberOfElements = (arr) => {
  const newthing = arr.reduce( (accumulator, val, idx) => {
    accumulator = 1
...
const reversedString = (str) => {
  let x = str.split('');
  let val = x.reduce((acc, val)=> val + acc, '');
  return val ;
};
...
const countNumberOfChildren = (arr) => {
  let newVal = arr.reduce( (accumulator, val, idx) => {
    if(val.children){
     accumulator = accumulator + val.children.length;
