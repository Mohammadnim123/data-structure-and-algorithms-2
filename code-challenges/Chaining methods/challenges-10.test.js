...
const createServer = () => {
  ...
  app.get('/aboutme', (request, response) => {
    response.status(200).json('my name is mohammad');
  });
  
  app.get('/favoritefoods', (request, response) => {
    response.status(200).json('[mansaf , shishbarak]');
  });
...
const count = (target, input) => {
  ...
  const newthing = element.reduce( (accumulator, val, idx) => {
    if(val == target){
...
const totalSum = (input) => {
 ...
 let newthing = elem.reduce( (accumulator, val, idx) => {
   return accumulator + val;
 }, 0);
 final = final + newthing
  })
  return final;
 };
...
const divisibleByFiveTwoToThePower = (input) => {
 newArr = [];
  input.forEach(elem => {
    ...
    let newthing = elem.reduce( (accumulator, val , idx) => {
      if(val%5 == 0 && Number.isInteger(val)){
        accumulator.push(Math.pow(2, val))
      }
      return accumulator;
    }, []);
    newArr.push(newthing);
 })
 return newArr;
};
...
let findShortest = (data) => {
  var min = Math.min.apply(Math, data.name.map(function(str) { return str.length; }));
return min;
};
